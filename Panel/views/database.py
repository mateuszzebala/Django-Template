...
class Database:
    ...
    def show(request, name, id):
        ...
        for i in fields.keys():
            field_type = model._meta.get_field(i).get_internal_type()
            if field_type == "ForeignKey":
                fields[i]['key'] = True
                fields[i]['link'] = reverse("Panel:database-show", kwargs={"name":model._meta.get_field(i).remote_field.model.__name__, "id":fields[i]['value'].pk})
        ...
        return render(request, "Panel/Pages/database/show.html", {
            ...
            "path": {
                "DATABASE": reverse("Panel:database"),
                model.__name__: reverse("Panel:database-model", kwargs={"name": model.__name__}),
                str(item.pk): reverse("Panel:database-show", kwargs={"name": model.__name__, "id": item.pk})
            }
        })
